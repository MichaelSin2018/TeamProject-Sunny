...
class Politics extends Component {
  ...
  componentDidMount() {
    axios.get(`http://13.125.111.99/articles/?category_id=1`)
        .then(politics => {
          this.setState({politics: politics.data})
        })
  ...
  // unique key fix needed
  render() {
    moment.locale('ko')
    let displayArticles = this.state.politics.map(article => {
        return (
          <div>
          <div className='card bg-light mb-2' key={`political-${article.id}`} onDoubleClick={() => this.playAudio(article.id, article.title)}>
            ...
            {console.log(`political-${article.id}`)}
            </div>
            <audio id={article.id} preload='none' controls >
