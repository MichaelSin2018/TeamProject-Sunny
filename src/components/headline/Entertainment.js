...
class Entertainment extends Component {
  ...
  componentDidMount() {
    axios.get(`http://13.125.111.99/articles/?category_id=8`)
        .then(entertainment => {
          this.setState({entertainment: entertainment.data})
        })
  ...
  render() {
    moment.locale('ko')
    let displayArticles = this.state.entertainment.map(article => {
        return (
          <div>
